...
async function main(args){
	if (args.puppeteerConfig.dowloadChromeVersion){
		...
		const revisionInfo = await browserFetcher.download(args.puppeteerConfig.chromeVersion, (download, total) => {
	...
	if (page.length > 0) {
		page = page[0];
		page.setUserAgent(args.puppeteerConfig.userAgent);
		await page.goto(args.puppeteerConfig.WAUrl, {
			waitUntil: 'networkidle0',
			timeout: 0
		});
		await page.setViewport({width: args.puppeteerConfig.width, height: args.puppeteerConfig.heigth, deviceScaleFactor: 1});
